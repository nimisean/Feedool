...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST['password'])) {
   ...
   if (empty($_POST["tick"])) {
    ...
    $sql = "INSERT INTO crudentials (username,email,password,image,balance,win,draw,loss,games) VALUES (:username,:email, :password,:image,:balance,:win,:draw,:loss,:games)";
    ...
    $stmt->bindValue(':win', $win);
    $stmt->bindValue(':draw', $draw);
    $stmt->bindValue(':loss', $loss);
    $stmt->bindValue(':games', $games);
    $result = $stmt->execute();
